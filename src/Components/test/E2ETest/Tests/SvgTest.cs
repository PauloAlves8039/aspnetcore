...
namespace Microsoft.AspNetCore.Components.E2ETest.Tests
{
    public class SvgTest : ServerTestBase<ToggleExecutionModeServerFixture<Program>>
    {
        ...
        [Fact]
        public void CanRenderWithNestedExternalSvg()
        {
            var appElement = Browser.MountTestComponent<SvgComponent>();

            var svgElement = appElement.FindElement(By.Id("svg-with-external-nested-svg"));
            Assert.NotNull(svgElement);

            var svgCircleElement = svgElement.FindElement(By.TagName("ellipse"));
            Assert.NotNull(svgCircleElement);
        }

        [Fact]
        public void CanRenderSvgWithDataReference()
        {
            var appElement = Browser.MountTestComponent<SvgComponent>();

            var svgElement = appElement.FindElement(By.Id("svg-with-data-reference"));
            Assert.NotNull(svgElement);

            var svgImageElement = svgElement.FindElement(By.TagName("image"));
            Assert.NotNull(svgImageElement);
        }
        ...
        public void CanRenderSvgWithScopedCSS()
        {
            ...
            Assert.Equal("green", svgCircleElement.GetCssValue("fill"));
